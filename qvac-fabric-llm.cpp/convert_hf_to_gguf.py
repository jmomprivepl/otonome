...
class BitnetModel(TextModel):
    ...
    def set_vocab(self):
        # BitNet-b1.58-2B-4T (HF) ships Llama-3-class tiktoken BPE in tokenizer.json (e.g. 128256
        # entries, ignore_merges). Do not use _set_vocab_gpt2() here: get_vocab_base_pre() can hash
        # to "gpt-2", which desyncs inference from the real pretokenizer. llama.cpp still stores
        # BPE as tokenizer.model "gpt2" but selects Llama-3 logic via tokenizer.pre "llama-bpe".
        if (self.dir_model / "tokenizer.model").is_file():
            self._set_vocab_sentencepiece()
        elif (self.dir_model / "tokenizer.json").is_file():
            self._set_vocab_bitnet_hf_bpe()
        else:
            self._set_vocab_gpt2()

    def _set_vocab_bitnet_hf_bpe(self) -> None:
        tokens, toktypes, _detected_pre = self.get_vocab_base()
        logger.info(
            "BitNet: using Hugging Face token order with tokenizer.ggml.pre=%r (ignoring detected %r).",
            "llama-bpe",
            _detected_pre,
        )
        self.gguf_writer.add_tokenizer_model("gpt2")
        ...
        merge_vocab = gguf.SpecialVocab(self.dir_model, load_merges=True)
        if merge_vocab.merges:
            merge_vocab.add_to_gguf(self.gguf_writer)
        else:
            # Llama-3-style BPE ignores merge rules at runtime, but llama.cpp still requires the key.
            self.gguf_writer.add_token_merges([])
            meta_vocab = gguf.SpecialVocab(self.dir_model, load_merges=False)
            meta_vocab.add_to_gguf(self.gguf_writer)
    ...
    def set_gguf_parameters(self):
        ...
        # Microsoft BitNet b1.58-2B-4T: ReLU² on gate branch, Sub-LayerNorm (RMSNorm on attn/ffn sub-paths).
        # Keys are consumed by llama.cpp / tooling to select the correct forward (see models/bitnet.cpp).
        hidden_act = self.hparams.get("hidden_act", "relu2")
        if not isinstance(hidden_act, str):
            hidden_act = getattr(hidden_act, "__name__", str(hidden_act))
        self.gguf_writer.add_string("bitnet.activation_type", hidden_act)
        self.gguf_writer.add_string("bitnet.normalization_type", "subln")
        # Weight tensors use RMSNorm (T5-style); sub-layer placements: attn_sub_norm, ffn_sub_norm.
        self.gguf_writer.add_string("bitnet.norm_impl", "rms")
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.endswith(".weight"):
            ...
            if scale_tensor is not None:
                ...
                scale_value = float(scale_tensor.reshape(-1)[0].item())
                if not math.isfinite(scale_value) or scale_value <= 0:
                    raise ValueError(
                        f"Invalid BitNet weight_scale for {name!r}: expected finite alpha > 0, got {scale_value!r}"
                    )
                # Dequantize once: packed ternary {-1,0,1} × HF per-tensor α. Downstream (e.g. TQ1_0) recomputes
                # its own per-block scales from these floats — do not apply α again during quantization.
