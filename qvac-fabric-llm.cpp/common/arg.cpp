...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--persona-plugin"}, "FILE",
        "load QVP1 persona plugin (snapshot/merge/restore on blk.L.attn_q.weight by default); see docs/persona-plugin-format.md",
        [](common_params & params, const std::string & value) {
            params.persona_plugin_path = value;
        }
    ).set_examples({LLAMA_EXAMPLE_MAIN}));
    add_opt(common_arg(
        {"--persona-layer"}, "N",
        "override block index L for default tensor name blk.L.<suffix> (default: from plugin header or 0 for dry-run)",
        [](common_params & params, int value) {
            params.persona_layer = value;
        }
    ).set_examples({LLAMA_EXAMPLE_MAIN}));
    add_opt(common_arg(
        {"--persona-tensor-suffix"}, "NAME",
        "tensor suffix when plugin header suffix is empty (default: attn_q.weight)",
        [](common_params & params, const std::string & value) {
            params.persona_tensor_suffix = value;
        }
    ).set_examples({LLAMA_EXAMPLE_MAIN}));
    add_opt(common_arg(
        {"--persona-dry-run"},
        "print tensor info and memcpy round-trip for blk.L.suffix, then exit (no generation)",
        [](common_params & params) {
            params.persona_dry_run = true;
        }
    ).set_examples({LLAMA_EXAMPLE_MAIN}));
    add_opt(common_arg(
        {"--nsdar-vector"}, "CSV",
        "ternary coefficients (-1,0,1) for adapter files 0.bin,1.bin,... under --nsdar-adapters-dir (F32 target tensor only)",
        [](common_params & params, const std::string & value) {
            params.nsdar_vector = value;
        }
    ).set_examples({LLAMA_EXAMPLE_MAIN}));
    add_opt(common_arg(
        {"--nsdar-adapters-dir"}, "DIR",
        "directory of flat binary adapters (see --nsdar-vector); each file size = nelements×4 (f32) or nelements×1 (i8)",
        [](common_params & params, const std::string & value) {
            params.nsdar_adapters_dir = value;
        }
    ).set_examples({LLAMA_EXAMPLE_MAIN}));
    add_opt(common_arg(
        {"--nsdar-layer"}, "N",
        "FFN block index L for tensor blk.L.<suffix> (default: 0)",
        [](common_params & params, int value) {
            params.nsdar_layer = value;
        }
    ).set_examples({LLAMA_EXAMPLE_MAIN}));
    add_opt(common_arg(
        {"--nsdar-ffn-suffix"}, "NAME",
        "tensor suffix after blk.L. (default: ffn_down.weight); use ffn_up.weight or a full blk.* name to override",
        [](common_params & params, const std::string & value) {
            params.nsdar_ffn_suffix = value;
        }
    ).set_examples({LLAMA_EXAMPLE_MAIN}));
    add_opt(common_arg(
        {"-i", "--interactive"},
