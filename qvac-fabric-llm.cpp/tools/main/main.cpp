...
int main(int argc, char ** argv) {
    ...
    if (params.persona_dry_run) {
        char tname[256];
        const int L = params.persona_layer >= 0 ? params.persona_layer : 0;
        snprintf(tname, sizeof(tname), "blk.%d.%s", L, params.persona_tensor_suffix.c_str());
        const bool ok = persona_session::dry_run_tensor(model, tname);
    ...
    persona_session g_persona;
    const bool use_persona = !params.persona_plugin_path.empty();
    struct persona_guard {
        persona_session * ps;
        bool en;
        ~persona_guard() {
            if (en && ps) {
    ...
    } persona_gr{ &g_persona, false };

    if (use_persona) {
        if (!g_persona.apply_from_file(model, params.persona_plugin_path, params.persona_layer, params.persona_tensor_suffix)) {
            return 1;
        }
        persona_gr.en = true;
    }

    // NSDAR: snapshot + linear adapter merge on one F32 tensor; restore when `g_nsdar` goes out of scope (end of main).
    // Hook point: after model load / persona apply, *before* the main generation loop (which calls `llama_decode`).
    // We keep merged weights for the whole autoregressive run; restoring after every `llama_decode` would undo the patch between tokens.
    qvac_memory_patcher::session g_nsdar;
    const bool use_nsdar = !params.nsdar_vector.empty() && !params.nsdar_adapters_dir.empty();
    if (use_nsdar) {
        std::string nsdar_err;
        const std::vector<int8_t> coeffs = qvac_memory_patcher::parse_nsdar_vector(params.nsdar_vector, nsdar_err);
        if (!nsdar_err.empty()) {
            LOG_ERR("%s: NSDAR: %s\n", __func__, nsdar_err.c_str());
            return 1;
        }
        const std::string nsdar_tname =
            qvac_memory_patcher::make_nsdar_tensor_name(static_cast<int>(params.nsdar_layer), params.nsdar_ffn_suffix);
        if (!g_nsdar.init(model, nsdar_tname)) {
            return 1;
        }
        if (!g_nsdar.apply(coeffs, params.nsdar_adapters_dir, nsdar_err)) {
            LOG_ERR("%s: NSDAR: %s\n", __func__, nsdar_err.c_str());
            return 1;
        }
    }
